...
%------------- BEGIN CODE --------------
   ...
   methods (Static)
   		...
   		function [heartbeats] = ecg_to_heartbeats(ecg_values, varargin)
			...
			addParameter(p, 'min_peak_height', 0.3);
			...
			min_peak_dist = 0.8 * 60/heart_rate;
   		...
   		function [scalogram, x, x_noabs] = sensors_to_scalogram(sensors_values, varargin)
			...
			switch signal_axis
				...
				case 'pca3'
					% [sensors_values, var_ret, U, S] = Helper.pca(sensors_values);
					[sensors_values, var_ret, U, S] = Helper.pca(abs(sensors_values));
					sensors_values = sensors_values(:, 3);
				case 'x'
					sensors_values = x;
				case 'y'
					sensors_values = y;
