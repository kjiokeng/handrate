classdef Handrate_combi
% Handrate_combi - A class which implements Handrate_combi
% 
   ...
   methods (Static)
   		...
   		function [hrv_features_matrix] = compute_hrv_features_batch(directory, varargin)
			...
			for idx=1:length(files_ids)
				...
				hrv_features = Handrate_combi.compute_hrv_features(heartbeats);
		...
		function [pred_hrv_features_matrix, gt_hrv_features_matrix] = compute_hrv_features_from_files(predictions_directory, n_files, varargin)
			...
			[pred_heartbeats, gt_heartbeats] = Handrate_combi.read_heartbeats_from_files(predictions_directory, n_files);
			...
			for idx=1:length(files_ids)
				...
				pred_hrv_features = Handrate_combi.compute_hrv_features(preds, ...
										'resample_freq', resample_freq, ...
										'min_peak_distance', 0.65);
				gt_hrv_features = Handrate_combi.compute_hrv_features(gts, ...
										'resample_freq', resample_freq, ...
			...
			toto = [gt_hrv_features_matrix(:, 1), pred_hrv_features_matrix(:,1)]
		end
		...
		function [X, Y, correlations] = compute()
			...
			% X = Handrate_combi.compute_hrv_features_batch(directory, ...
			% 							  'metadata_file', metadata_file, ...
			...
			predictions_directory = '../python/predictions/handrate.rnn.conv.3s.10ones.128units-pca3.h5/';
			n_files = 44;
			resample_freq = 100;
			[X, Y] = Handrate_combi.compute_hrv_features_from_files(predictions_directory, ...
										  n_files, ...
										  'resample_freq', resample_freq);
		...
		function [X, Y] = main(varargin)
			% main - Main function
			% 	
			% Syntax:  [] = Handrate_combi.main(opt)
			%------------- BEGIN CODE --------------

			[X, Y] = Handrate_combi.compute();			
		end
   end
end