classdef Handrate_eval
% Handrate_eval - A class which implements Handrate_eval
% 
...
%------------- BEGIN CODE --------------
   ...
   methods (Static)
   		...
   		function [ibi, hr] = compute_ibi_hr_xcorr(x, varargin)
			...
			[ibi, hr] = Handrate_eval.compute_ibi_hr(autocor, ...
   		...
   		function [ibi, hr] = compute_ibi_hr_wavelet(x, varargin)
			...
			[ibi, hr] = Handrate_eval.compute_ibi_hr(wt, ...
						'resample_freq', resample_freq);
   		end
   		...
   		function [] = evaluate_config(directory, varargin)
			...
			for idx=1:length(files_ids)
				...
				% Ground truth
				[ibi_gt, hr_gt] = Handrate_eval.compute_ibi_hr(heartbeats, ...
								'resample_freq', resample_freq);
				ibi_gts(end+1) = ibi_gt;
				hr_gts(end+1) = hr_gt;

				% FFT
				[ibi_fft, hr_fft] = Handrate_eval.compute_ibi_hr_fft(x_noabs, ...
								'resample_freq', resample_freq);
				ibi_ffts(end+1) = ibi_fft;
				hr_ffts(end+1) = hr_fft;

				% xcorr
				[ibi_xcorr, hr_xcorr] = Handrate_eval.compute_ibi_hr_xcorr(x_noabs, ...
								'resample_freq', resample_freq);
				ibi_xcorrs(end+1) = ibi_xcorr;
				hr_xcorrs(end+1) = hr_xcorr;

				% wavelet
				[ibi_wavelet, hr_wavelet] = Handrate_eval.compute_ibi_hr_wavelet(x_noabs, ...
								'resample_freq', resample_freq);
		...
		function [] = evaluate()
			...
			Handrate_eval.evaluate_config(directory, ...
										  'metadata_file', metadata_file, ...
		...
		function [X, Y] = main(varargin)
			% main - Main function
			% 	
			% Syntax:  [] = Handrate_eval.main(opt)
			%------------- BEGIN CODE --------------

			Handrate_eval.evaluate();			
		end
   end
end