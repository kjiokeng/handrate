...
class SoftDTWTF():
    ...
    def _soft_dtw_core(D):
        ...
        if SoftDTWTF.A is None:
            SoftDTWTF.A = tf.Variable(zeros, dtype=tf.float64)
        
        if SoftDTWTF.B is None:
            SoftDTWTF.B = tf.Variable(zeros, dtype=tf.float64)

        SoftDTWTF.A = SoftDTWTF.A[1:m+1,0].assign(tf.ones_like(SoftDTWTF.A[1:m+1,0])*DBL_MAX)
        SoftDTWTF.B = SoftDTWTF.B[0,1:n+1].assign(tf.ones_like(SoftDTWTF.B[0,1:n+1])*DBL_MAX)
        R_single = SoftDTWTF.A + SoftDTWTF.B
