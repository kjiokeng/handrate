...
def get_trained_models(X_train, Y_train, X_dev, Y_dev, dropout_rate=0.2):
    ...
    # encoder_1 = Bidirectional(GRU(n_units, return_sequences=True, name="encoder_lstm_1"), name="bidirectional_encoder_lstm_1")
    # encoder_1_outputs = encoder_1(X)

    # encoder_2 = Bidirectional(GRU(n_units, return_sequences=True, return_state=True, name="encoder_lstm_2"), name="bidirectional_encoder_lstm_2")
    # encoder_out, encoder_fwd_state, encoder_back_state = encoder_2(X)
    encoder_2 = GRU(n_units, return_sequences=True, return_state=True, name="encoder_lstm_2")
    encoder_out, encoder_fwd_state = encoder_2(X)


    #################### Decoder network ####################
    decoder_inputs = Input(shape=(None, 1), name="target_output")
    # Set up the decoder GRU, using `encoder_states` as initial state.
    # decoder_gru = GRU(n_units*2, return_sequences=True, return_state=True, name='decoder_gru')
    ...
    decoder_out, decoder_state = decoder_gru(
        ...
        decoder_inputs, initial_state=encoder_fwd_state
    )
    ...
    model_id = 'handrate.rnn.conv.attn4.3s.{:d}ones.{:d}units'.format(n_ones, n_units)
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_f1', mode='max', factor=0.1, patience=5, min_lr=0.0001, verbose=1)
