...
def train_config(n_seconds, n_ones, signal_axis, n_units, denoising_method=""):
    ...
    model_filename = 'handrate-model-stacked-{:d}s-{:d}ones-{:s}-{:d}units.h5'.format(n_seconds, n_ones, signal_axis, n_units)
    history_filename = 'models/handratenn/handrate-model-{:d}s-{:d}ones-{:s}-{:d}units-history.pkl'.format(n_seconds, n_ones, signal_axis, n_units)
    ...
    checkpointer = ModelCheckpoint(filepath=model_checkpoint_filename, monitor="loss", verbose=1, save_best_only=True)
    # earlystopping = EarlyStopping(monitor='val_loss', verbose=1, patience=500)
    hist = model.fit(X_train, Y_train, validation_data=(X_dev, Y_dev), epochs=1000, batch_size=32, callbacks=[checkpointer])
...
def handrate_model(input_shape, n_units=128):
    ...
    # Step 1: CONV layer
    # X = Conv1D(n_units, kernel_size=7, strides=1, padding="same")(X_input)
    ...
    X = GRU(units = n_units, return_sequences = True)(X)
    # X = Dropout(rate=0.2)(X)
    X = BatchNormalization()(X)
