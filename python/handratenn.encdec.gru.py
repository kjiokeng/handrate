...
from tensorflow.keras.layers import GRU, Bidirectional, BatchNormalization, Reshape, MaxPooling1D
...
datafile = '../matlab/datasets/handrate-dataset-3s-10ones-pca.mat'
# datafile = '../matlab/datasets/handrate-simu-dataset-3s-5ones-pcagoodsnr.mat'
# datafile = '../matlab/datasets/handrate-simu-dataset-3s-5ones-pcalowsnrdenoised.mat'
...
def get_trained_models(X_train, Y_train, X_dev, Y_dev, dropout_rate=0.2):
    ...
    X = encoder_inputs
    # X = Conv1D(n_units, kernel_size=10, strides=1)(X)
    # X = BatchNormalization()(X)
    # X = Activation('relu')(X)
    # X = Dropout(rate=dropout_rate)(X)


    encoder = GRU(n_units, return_state=True, name="encoder_lstm_2")
    encoder_outputs, state_h = encoder(X)
    encoder_states = state_h # Keep only the states
    ...
    decoder_lstm = GRU(n_units, return_sequences=True, return_state=True, name="decoder_lstm")
    ...
    # model_id = 'handrate.encdec.3s.{:d}ones.{:d}units'.format(n_ones, n_units)
    model_id = 'handrate.encdec.1s.{:d}ones.{:d}units'.format(n_ones, n_units)
    model_filename = model_id + '.h5'
    if False and (os.path.isfile(model_filename) or os.path.isdir(model_filename)):
    ...
    earlystopping = EarlyStopping(monitor='val_f1', mode='max', verbose=1, patience=20)
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_f1', mode='max', factor=0.1, patience=10, min_lr=0.001, verbose=1)
