...
datafile = '../matlab/datasets/handrate-spect-dataset-3.0s-10ones-pca.mat'
n_units = 256
n_ones = 10
...
def get_trained_models(X_train, Y_train, X_dev, Y_dev, dropout_rate=0.2):
    ...
    X = encoder_inputs
    X = Conv1D(int(n_units/4), kernel_size=21, strides=1, padding="same")(X)
    ...
    model_id = 'handrate.rnn.conv.spect.3s.{:d}ones.{:d}units'.format(n_ones, n_units)
    ...
    earlystopping = EarlyStopping(monitor='val_f1', mode='max', verbose=1, patience=200)
    ...
    reduce_lr = ReduceLROnPlateau(monitor='f1', mode='max', factor=0.1, patience=5, min_lr=0.00001, verbose=1)
