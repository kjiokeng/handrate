...
def get_trained_models(X_train, Y_train, X_dev, Y_dev, dropout_rate=0.2):
    ...
    X = encoder_inputs
    X = Conv1D(n_units*2, kernel_size=9, strides=1, padding="same")(X)
    X = BatchNormalization()(X)
    X = Activation('relu')(X)
    X = Dropout(rate=dropout_rate)(X)

    encoder_1 = LSTM(n_units, return_sequences=True, name="encoder_lstm_2")
    encoder_outputs = encoder_1(X)

    encoder = LSTM(n_units, return_state=True, name="encoder_lstm_stacked")
    ...
    model_id = 'handrate.rnn.conv.stacked.3s.{:d}ones.{:d}units'.format(n_ones, n_units)
    ...
    earlystopping = EarlyStopping(monitor='val_f1', mode='max', verbose=1, patience=50)
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_f1', mode='max', factor=0.1, patience=15, min_lr=0.0001, verbose=1)
