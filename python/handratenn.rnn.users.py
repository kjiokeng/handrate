...
def get_trained_models(X_train, Y_train, X_dev, Y_dev, dropout_rate=0.2, username=''):
    # Useful variables
    ...
    model_id = 'handrate.rnn.conv.users.{:s}.3s.{:d}ones.{:d}units'.format(username, n_ones, n_units)
    ...
    checkpointer = ModelCheckpoint(filepath=model_filename, monitor="val_loss", mode='min', verbose=1)
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', mode='min', factor=0.1, patience=30, min_lr=0.0001, verbose=1)
...
def train_for_user(username):
    ...
    datafile = '../matlab/datasets/users/handrate-dataset-{:s}-3s-10ones-pca.mat'.format(username)
    ...
    full_model, encoder_model, decoder_model = get_trained_models(X_train, Y_train,
        X_dev, Y_dev, dropout_rate=dropout_rate, username=username)
...
if __name__ == '__main__':
    ...
    for username in usernames:
        ...
        print("-----------------------------------------------------")
        print("-----------------------------------------------------")
        print("------ Training for user {:s} -----".format(username))
        print("-----------------------------------------------------")
        print("-----------------------------------------------------")
        print("-----------------------------------------------------")
        train_for_user(username)